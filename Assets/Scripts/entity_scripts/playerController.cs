...
public class playerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ammocapacity = CEO_script.ammocapacity;
        staminacap = CEO_script.staminacap;
        //invincible=false;
        ...
        if(currenProj !=null)
        {
            ...
            ammoRate =currenProj.GetComponent<Bullet>().ammoCost;
            inaccuracyFloat = currenProj.GetComponent<Bullet>().inaccuracyFloat;
        }
        switchweapons();
    }
    ...
    void FixedUpdate()
    {
        ...
        if(looking.normalized.x>1/Mathf.Sqrt(2) && !isDashing)
            facingX=-1;
        else if(looking.normalized.x<-1/Mathf.Sqrt(2) && !isDashing)
            facingX=1;
        else
            facingX=0;

        if(looking.normalized.y>1/Mathf.Sqrt(2) && !isDashing)
            facingY=-1;
        else if(looking.normalized.y<-1/Mathf.Sqrt(2) && !isDashing)
            facingY=1;
        else
            facingY=0;
        ...
        } else if (!Input.GetButton("Fire1") && ammo < ammocapacity && Time.time > lastFireTime + reloadTime){   //ammo recharge
	        ammo++;
            ammoBar.setHealth(ammo);        //ammo, stamina bar set
        }
        ...
        {
            //Dash trigger
            if (Input.GetButton("Fire2") && (Time.time > lastRollTime + rollReload) && stamina >= dashUsage)
            {
                invincible = true;
                isDashing=true;
        ...
        if (invincible && isDashing)
        {
            ...
            cameraShake.instance.shakeCamera(1f*(stamina-dashUsage)/staminacap,rollDuration);
    ...
    void FireWeapon(Vector3 position, Quaternion rotation)
    {
        ...
        if (currenProj.name == "shotgun")
        {
            ...
            if(CEO_script.currentGameState==CEO_script.gameState.bossBattle)
                deviation /= 10;
            else
                deviation /= 5;
            //moved this from the update loop
